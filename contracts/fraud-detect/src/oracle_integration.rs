...
/// Update fraud detection result from oracle (only callable by oracle contract)
pub fn update_fraud_result_from_oracle(
    ...
    // Store the result (we'll use a simplified approach - in production this would be more sophisticated)
    let result_key = soroban_sdk::Symbol::short("fraud_res");
    let result_data = (risk_score, is_fraudulent, model_hash);
    env.storage().persistent().set(&(result_key, account), &result_data);
    
    // Emit event for traceability
    crate::events::emit_fraud_result_updated(env, account, risk_score, model_hash, is_fraudulent);
}
